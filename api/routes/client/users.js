...
router.get("/:id", auth, async (req, res) => {
  try {
    const data = await User.findOne({ _id: req.params.id }).select(
      "-password -login -userType -online -start -end"
    );
...
router.get("/", async (req, res) => {
  ...
  try {
    const data = await User.findOne({ _id: req.query.userId }).select(
      "-password -login -userType -online -start -end"
    );
    ...
    if (from !== null && to !== null && s) {
      if (
        new Date(date).getTime() >= from.getTime() &&
        new Date(date).getTime() <= to.getTime()
      ) {
        data._id = s;
        isVacation = true;
      }
