...
router.post(
  "/",
  [
    check("firstname", "Введите фамилию")
      .trim()
      .toUpperCase()
      .isLength({ min: 2 }),
    check("lastname", "Введите Имя").trim().toUpperCase().isLength({ min: 2 }),
    check("phone", "Введите номер телефона").not().isEmpty().trim(),
    check(
      "phone",
      "Вы ввели некорректный номер. Длина номера должна быть от 5 до 11 знаков"
    ).isLength({
      min: 5,
      max: 11,
    }) /* , check("phone", "Номер телефона должен содержать только цифры").isNumeric() */,
  ],
  auth,
  async (req, res) => {
    ...
    try {
      ...
      const isExist = await Ticket.findOne({
        $and: [{ date: req.body.date }, { user: req.body.employee }],
      });
      ...
      if (ticketId) {
        ...
        if (isExistId) {
          try {
            const data = await Ticket.replaceOne(
              { _id: ticketId },
              { ...query }
            );
...
router.get("/checkTime/:employeeId/:time/", auth, async (req, res) => {
  ...
  try {
    ...
    const checking = tickets.filter((ticket) => {
      return (
        ticket.date.toTimeString().slice(0, 5) === time &&
        ticket.isBusy === true
      );
    });
...
router.patch("/:id", auth, async (req, res) => {
  try {
    ...
    res.status(500).json({
      message:
        "Ошибка обновления информации о талоне. Попробуйте еще раз или обратитесь к разработчику",
    });
  }
});
